...
{
    filename=$1
    valgrind_output=$(valgrind --leak-check=full --error-exitcode=1 ./pathfinder $filename 2>&1)

    if [ $? -eq 0 ]; then
        echo "No memory leaks detected."
    else
        echo "****************************"
        echo "Memory leaks detected or errors found."
        echo "Input file: ${filename}"
        echo "****************************"
    fi
}
...
echo "Valgrind Memory Leaks Check:"
